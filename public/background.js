...
browser.storage.local.set({aiReplyApiKey});
browser.storage.local.set({aiReplyApiAddress});
browser.storage.local.set({aiReplyPromptMain});
browser.storage.local.set({aiReplyPromptFormat});
browser.storage.local.set({aiReplyModel});
...
browser.menus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "ai-reply-with-the-chat") {
    ...
    if (info.selectedMessages.messages[0]) {
      ...
      browser.messages.getFull(email.messageId).then((message) => {
...
async function fetchEmailContentFullAndRaw() {
  ...
  try {
    ...
    email.fullMessage = await browser.messages.getFull(email.messageId);
  ...
  try {
    ...
    email.rawMessage = await browser.messages.getRaw(email.messageId);
  } catch (err) {
...
async function fetchAIReply(prompt) {
  ...
  const settings = await browser.storage.local.get({aiReplyApiKey, aiReplyApiAddress, aiReplyModel});
...
async function closePopupAndOpenReplyWindow(aiGeneratedReply) {
  ...
  try {
    // let tab = await browser.compose.beginReply(email.messageId, "replyToAll", {isPlainText: true, plainTextBody: newBody, isModified: true});

    // console.log("window type: ", window.type, "tab id: ", tab.id);

    // let details = await browser.compose.getComposeDetails(tab.id);

    let tab = await browser.compose.beginReply(email.messageId, "replyToAll", {
      isPlainText: true,
      plainTextBody: newBody
    });

    console.log("Neuer Compose-Tab ID:", tab.id);
  ...
  // let newBody = aiGeneratedReply + email.fullMessage.parts[0].body;
  //let tab = await browser.compose.beginReply(email.messageId, "replyToAll"); //, {isPlainText: false, body: newBody, isModified: true});
  // try {
  //   let tab = await browser.compose.beginReply(email.messageId, "replyToAll"); //, {isPlainText: true, plainTextBody: newBody, isModified: true});
  //   return { success: true, tabId: tab.id };
  // } catch (error) {
  ...
  // Body aktualisieren
  // await browser.compose.setComposeDetails(tab.id, { body: newBody, isPlainText: true, isModified: true });
